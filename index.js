...
client.on("interactionCreate", async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "sticky") {
      ...
      return interaction.reply({ content: `📌 Sticky set in ${channel} every **${minutes} minute(s)**.`, flags: MessageFlags.Ephemeral });
    }
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "settranscripts") {
      if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageChannels)) return interaction.reply({ content: "❌ You need Manage Channels permission.", flags: MessageFlags.Ephemeral });
      const channel = interaction.options.getChannel("channel", true); featureData.transcriptChannelId = channel.id; saveFeatureData();
      return interaction.reply({ content: `📄 Ticket transcripts will be sent to ${channel}.`, flags: MessageFlags.Ephemeral });
    }
    if (interaction.isChatInputCommand() && interaction.commandName === "announce") {
      ...
      return interaction.reply({ content: `📢 Announcement sent to ${channel}.`, flags: MessageFlags.Ephemeral });
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith("product_")) {
      ...
      if (!product) return interaction.reply({ flags: MessageFlags.Ephemeral, content: "That product is no longer available." });
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith("order_modal_")) {
      ...
      if (!Number.isInteger(quantity) || quantity < 1 || quantity > 100) {
        await interaction.reply({ flags: MessageFlags.Ephemeral, content: "Please enter a quantity from 1 to 100." });
        return;
      }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "payment_method") {
      ...
      if (!order) {
        await interaction.reply({ flags: MessageFlags.Ephemeral, content: "Your order session expired. Please start again with /shop." });
        return;
      }
      ...
      // STAFF_ROLE_ID supports multiple role IDs separated by commas.
      // Example: STAFF_ROLE_ID=123456789012345678,234567890123456789,345678901234567890
      const staffRoleIds = (process.env.STAFF_ROLE_ID || "")
        .split(",")
        .map(id => id.trim())
        .filter(Boolean);
      const ticketCategoryId = process.env.TICKET_CATEGORY_ID?.trim();

      if (!staffRoleIds.length || !ticketCategoryId) {
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
      ...
      // Fetch every staff role, the category, and the bot member.
      const staffRoles = [];
      ...
      if (staffRoles.length !== staffRoleIds.length) {
        const found = new Set(staffRoles.map(role => role.id));
        const missing = staffRoleIds.filter(id => !found.has(id));
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
          content: `❌ These STAFF_ROLE_ID value(s) are invalid or inaccessible: ${missing.join(", ")}`
        });
        return;
      }
      ...
      if (!ticketCategory || ticketCategory.type !== ChannelType.GuildCategory) {
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
          content: "❌ TICKET_CATEGORY_ID is invalid or does not point to a category."
        });
        return;
      }
      ...
      if (!botMember) {
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
          content: "❌ I could not resolve my bot member in this server."
        });
        return;
      }
      ...
      if (!botMember.permissions.has(PermissionFlagsBits.ManageChannels)) {
        await interaction.reply({
          flags: MessageFlags.Ephemeral,
          content: "❌ I need the **Manage Channels** permission to create order tickets."
        });
        return;
      }
    ...
    if (interaction.isButton() && interaction.customId === "close_ticket") {
      ...
      if (!isStaff && !isTicketOwner) {
        await interaction.reply({ flags: MessageFlags.Ephemeral, content: "You cannot close this ticket." });
        return;
      }
    ...
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ flags: MessageFlags.Ephemeral, content: "Something went wrong. Check the Railway logs." }).catch(() => {});
    }
  }
});
